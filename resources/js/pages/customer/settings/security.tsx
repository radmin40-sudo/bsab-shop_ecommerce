...
export default function CustomerSecuritySettings() {
    return (
        <>
            <Head title="Security settings" />
            <PortalLayout role="customer" title="Security" eyebrow="Account preferences">
                <div className="mb-6">
                    <Link
                ...
                <div className="rounded-3xl border border-[#def0e2] bg-white p-6 shadow-[0_12px_28px_rgba(22,59,36,0.04)]">
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#edf7ff] text-[#1d6fa8]">
                        <ShieldCheck size={24} />
                    </div>
                    <h1 className="font-display mt-5 text-3xl font-bold text-[#163b24]">Security</h1>
                    <p className="mt-3 max-w-2xl text-sm leading-6 text-[#647568]">
                        Review how your account is protected and manage the measures that help keep your information safe.
                    </p>

                    <div className="mt-6 space-y-4">
                        {['Two-factor authentication', 'Login alerts', 'Trusted devices', 'Account recovery email', 'Session security checks'].map(
                            (item) => (
                                <div
                                    key={item}
